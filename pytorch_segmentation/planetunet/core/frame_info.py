...
def image_normalize(im, method, axis=(0, 1), c=1e-8, nodata_val=None, means=None, stds=None):
    """
    ...
    """
    if method is None or method.lower() in ["nonorm", "none"]:
        return im

    # Convert nodata values to nan, so they are handled correctly by nanmean/nanstd
    if nodata_val is not None and np.sum(im == nodata_val) > 0:
        im = im.astype(np.float32)
        im[np.any(im[..., :4] == nodata_val, axis=2), :] = np.nan  # treat whole pixel as nodata if any band has nodata value

    # Normalization by mean and stardard dev, per channel. Stats either of this image, or passed with 'means' and 'stds'
    if method.lower().replace("_", "") in ["stddev", "std", "tilenorm"]:
        if means is None:
            means = np.nanmean(im, axis)
        if stds is None:
            stds = np.nanstd(im, axis)
        return (im - means) / (stds + c)

    elif method == "std_dev_except_coverband":
        im5 = im[..., 4]
        norm = (im - np.nanmean(im, axis)) / (np.nanstd(im, axis) + c)
        norm[..., 4] = im5
        return norm

    else:
        print(f"WARNIGN!! Unsupported normalize_method '{method}', skipping normalizing.")
        return im
...
class FrameInfo:
    ...
    def __init__(self, img, annotations, weight, norm_method, dtype=np.float32, fp=None, frame_id:int=None):
    ...
    def getPatch(self, i, j, patch_size, img_size, normalize=1.0):
        ...
        if normalize >= r[0]:
            im = image_normalize(im, method=self.norm_method, axis=(0, 1))
        ...
        # comb_img = np.concatenate((im, an, we, cv), axis=-1)
