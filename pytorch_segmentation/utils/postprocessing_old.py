...
def mosaic_to_raster(dataset,model_path,out_path,device_ids,bs=16,out_size=256,num_workers=4,pin_memory=True,dtype="uint8",compress="deflate"):
    ...
    for rank in range(world_size):
        if world_size > 1:
            ...
            d_id = device_ids[0]
        p = mp.Process(target=run_inference, args=(rank,d_id,world_size,dataset,model_path,mmap_file,out_size,bs,num_workers,pin_memory,dtype))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
...
def run_inference(rank,device_id,world_size,dataset,model_path,mmap_path,patch_size,bs,num_workers,pin_memory,dtype):
